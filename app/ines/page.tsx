...
export default function Directory() {
 ...
 useEffect(() => {
  Papa.parse("/ines.csv", {
    header: true,
    download: true,
    dynamicTyping: true, // ← transforme automatiquement en Number quand c'est possible
    skipEmptyLines: true, // ← évite les lignes vides
    complete: (result) => {
      const parsedData = result.data as MatchData[];

      setData(parsedData);

      // Calcul des totaux avec valeurs numériques
      const ptsTotal = parsedData.reduce((sum, match) => sum + (match.PTS || 0), 0);
      const astTotal = parsedData.reduce((sum, match) => sum + (match.AST || 0), 0);
      const rbdTotal = parsedData.reduce((sum, match) => sum + (match.RBD || 0), 0);
  ...
  const filterByCompetition = (data: MatchData[]) => {
    switch (competitionFilter) {
      case "Championnat":
        return data.filter((match) => match.CHAMPIONNAT === "NCAA");
      case "PlayOff":
        return data.filter(
